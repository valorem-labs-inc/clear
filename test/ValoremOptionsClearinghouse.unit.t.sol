...
contract ValoremOptionsClearinghouseUnitTest is BaseClearinghouseTest {
    ...
    function test_sweepFees() public {
        ...
        emit FeeSwept(address(WETHLIKE), FEE_TO, underlyingFee);
        emit FeeSwept(address(DAILIKE), FEE_TO, exerciseFee);

        vm.prank(FEE_TO);
        engine.sweepFees(tokens);

        // Balance assertions -- with tolerance of 1 wei for loss of precision
        assertApproxEqAbs(WETHLIKE.balanceOf(FEE_TO), underlyingFee, 1, "feeTo underlying balance after sweep");
        assertApproxEqAbs(DAILIKE.balanceOf(FEE_TO), exerciseFee, 1, "feeTo exercise balance after sweep");

        // Fee assertions -- 1 wei left in fee balance as gas optimization
